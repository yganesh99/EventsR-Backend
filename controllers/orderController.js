	...
	try {
		...
		if (!userId) {
			return res.status(400).json({
				error: 'Id of the user who placed the order is required',
			});
		}

		if (!products) {
			return res.status(400).json({
				error: 'An array of product objects each with a product id and a quantity is required.',
			});
		}
		...
		for (const { productId, quantity } of products) {
			...
			if (!product) {
				return res
					.status(400)
					.json({ error: `Product with Id not found: ${productId}` });
			}
